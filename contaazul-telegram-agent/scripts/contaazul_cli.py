...
Usage examples:
  ...
  python3 scripts/contaazul_cli.py --har /path/to/session.har statement --from 2026-02-01 --to 2026-02-28 --csv extrato.csv
"""
...
def _mask_document(doc: Optional[str]) -> str:
    if not doc:
        return ""
    if len(doc) <= 5:
        return doc
...
def cmd_list_people(ctx: AuthContext, args: argparse.Namespace) -> None:
    profile_map = {
        "customer": ["CUSTOMER"],
        "supplier": ["SUPPLIER"],
        "both": ["CUSTOMER", "SUPPLIER"],
    }
    profile_types = profile_map[args.profile]
    ...
    for profile_type in profile_types:
        ...
        while True:
            params = {
                ...
                "recover_legacy_id": "true",
                "textual_search_only": "true",
            }
            if args.status != "all":
                params["person_status"] = args.status
            r = req(ctx, "GET", "/contaazul-bff/person-registration/v2/persons", params=params)
            if r.status_code != 200:
                die_response(r)
            ...
            for it in items:
                ...
                person = people.setdefault(
                    pid,
                    {
                        ...
                        "personType": it.get("personType"),
                        "profiles": set(),
                    },
                )
                for p in it.get("profiles") or []:
                    person["profiles"].add(p.get("profileType") or p.get("profile") or p.get("type") or "")
                person["profiles"].add(profile_type)

            total_items = payload.get("totalItems", 0)
            if page * args.page_size >= total_items or not items:
                break
            page += 1
        ...
        rows.append(
            {
                "id": p.get("id"),
                "name": p.get("name"),
                "document": doc if args.show_document else _mask_document(doc),
                "active": p.get("active"),
                "personType": p.get("personType"),
                "profiles": sorted([x for x in p.get("profiles", set()) if x]),
            }
        )
    rows.sort(key=lambda x: (x.get("name") or "").lower())

    if args.raw:
        print(json.dumps({"count": len(rows), "people": rows}, ensure_ascii=False, indent=2))
        return

    print(f"count: {len(rows)}")
    for p in rows:
        print(
            f"- {p['name']} | doc {p['document']} | active={p['active']} | "
            f"type={p['personType']} | profiles={','.join(p['profiles'])} | {p['id']}"
        )
...
def cmd_statement(ctx: AuthContext, args: argparse.Namespace) -> None:
    body = {
    ...
    params = {"page": args.page, "page_size": args.page_size}
    r = req(ctx, "POST", "/finance-pro-reader/v1/financial-statement-view", params=params, json_body=body)
    if r.status_code != 200:
        die_response(r)
    ...
    if args.raw:
        ...
        if totals:
            print(
                "totals: "
                f"inflow={totals.get('inflow')} | "
                f"outflow={totals.get('outflow')} | "
        ...
        for it in payload.get("items", []):
            ...
            desc = it.get("description")
            iid = it.get("id")
            print(f"- {date} | {typ} | R$ {val} | saldo {bal} | {status} | {desc} | {iid}")
                ...
                w.writerow(
                    {
                        ...
                        "status": it.get("status"),
                        "id": it.get("id"),
                        "installmentId": it.get("installmentId"),
                        "financialEventId": it.get("financialEventId"),
                        "financialAccountId": it.get("financialAccountId"),
                    }
                )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # list-people
    s = sub.add_parser("list-people", help="Lista clientes/fornecedores")
    s.add_argument("--profile", choices=["customer", "supplier", "both"], default="both")
    s.add_argument("--search", default="")
    s.add_argument("--status", choices=["active", "inactive", "all"], default="active")
    s.add_argument("--page-size", type=int, default=50)
    s.add_argument("--show-document", action="store_true", help="Mostra CPF/CNPJ completo")
    s.add_argument("--raw", action="store_true")
    s.set_defaults(func=cmd_list_people)
    ...
    s = sub.add_parser("statement", help="Extrato de movimentações")
    ...
    s.add_argument("--page", type=int, default=1)
    s.add_argument("--page-size", type=int, default=50)
    s.add_argument("--csv", help="Path de saída CSV")
    s.add_argument("--raw", action="store_true")
